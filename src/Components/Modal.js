...
export const Modal = ({todo, editModal,updateTodoHandler}) => {
    ...
    const handleEditTodoSubmit=(e)=>{
        e.preventDefault();
        handleClose();
        updateTodoHandler(editTodo, todo.id);
        auth.onAuthStateChanged(user=>{
            if(user){
                db.collection('todos of ' + user.uid).doc(todo.id).update({
                    Todo: editTodo
                })
            }
            else{
                console.log('user is not signed in at modal.js to update todo');
            }
        })
    }
